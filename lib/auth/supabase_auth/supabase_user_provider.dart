...
class TrakerProGetelSupabaseUser extends BaseAuthUser {
  TrakerProGetelSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> trakerProGetelSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TrakerProGetelSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
